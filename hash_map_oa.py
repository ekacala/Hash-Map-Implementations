...
class HashMap:
    ...
    def find_open_index(self, array: DynamicArray, index: int) -> int:
        """
        Finds an open index using open addressing with quadratic probing.
        """
        count = 0
        new_index = index
        found_status = False
        ...
        while found_status is False:
            count += 1
            new_index = (index + count ** 2) % array.length()
            if new_index > array.length():
                new_index = (new_index - array.length())

            if array[new_index] is None:
                found_status = True
            elif array[new_index].is_tombstone is True:
                found_status = True
            elif array[new_index].key == key:
                found_status = True
